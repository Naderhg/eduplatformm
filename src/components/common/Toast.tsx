...
export const Toast: React.FC<ToastProps> = ({ id, message, type, onDismiss }) => {
  ...
  return (
    <div
      className={`toast toast-${type} flex items-center p-4 mb-2 rounded-lg shadow-lg`}
      role="alert"
    >
...
export const ToastContainer: React.FC<{ toasts: ToastProps[]; onDismiss: (id: string) => void }> = ({ toasts, onDismiss }) => {
  return (
    <div className="toast-container fixed bottom-4 right-4 z-50 w-full max-w-xs space-y-2">
      {toasts.map((toast) => (
        <Toast
          key={toast.id}
          id={toast.id}
